...
class Dukun2(Dukun):
    def __init__(self, gambar, lebar, tinggi, x, y):
        ...
        self.dukun2_shoot = pygame.image.load("assets/dukun/dukun_2_attack.png").convert_alpha()
        s_w = base_sihir.get_width()
        s_h = base_sihir.get_height()
        self.gambar_sihir = pygame.transform.scale(base_sihir, (int(s_w* 0.080), int(s_h* 0.080)))
        super().__init__(gambar, lebar, tinggi, x, y)
    ...
    def shoot(self, screen):
        ...
        if pygame.mouse.get_pressed()[0] and self.fired == False and pos[0] < 720 and pos[1] > 70:
            self.fired = True
            self.image = pygame.transform.scale(self.dukun2_shoot, (self.width, self.height))
            self.display(screen)
            sihir = Serangan(self.gambar_sihir, 685, 380, self.angle)
            group_serangan.add(sihir)
        # reset mouse
...
class Dukun3(Dukun):
    def __init__(self, gambar, lebar, tinggi, x, y):
        ...
        self.dukun3_shoot = pygame.image.load("assets/dukun/dukun_3_attack.png").convert_alpha()
        s_w = base_sihir.get_width()
        s_h = base_sihir.get_height()
        self.gambar_sihir = pygame.transform.scale(base_sihir, (int(s_w* 0.080), int(s_h* 0.080)))
        super().__init__(gambar, lebar, tinggi, x, y)
    ...
    def shoot(self, screen):
        ...
        if pygame.mouse.get_pressed()[0] and self.fired == False and pos[0] < 720 and pos[1] > 70:
            self.fired = True
            self.image = pygame.transform.scale(self.dukun3_shoot, (self.width, self.height))
            self.display(screen)
            sihir = Serangan(self.gambar_sihir, 685, 380, self.angle)
            group_serangan.add(sihir)
        # reset mouse
